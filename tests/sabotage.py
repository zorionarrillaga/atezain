...
SABOTAGES = [
    ...
    ("a date the triage cannot read is loaded anyway", "api/app.py",
     "        if bad is not None:\n            rejected.append(bad)\n            continue\n",
     "        if bad is not None:\n            rejected.append(bad)\n"
     "            dates.setdefault('issued', (row.get('issued') or '').strip())\n"
     "            dates.setdefault('due', (row.get('due') or '').strip())\n"),
    ...
    ("the amount convention is assumed instead of proved", "api/app.py",
     '            return "point" if j > i else "comma"\n    return ""\n',
     '            return "point" if j > i else "comma"\n    return "point"\n'),
    ("a date order the file never settled is used anyway", "api/app.py",
    ...
    ("the rate limit stops saying which limit it is", "api/app.py",
     ...
     '            f"{why}: "\n'),
    ("a refusal claims the other rows loaded when none did", "api/app.py",
     '            else " — the row was not loaded, and no row in this file was")\n',
     '            else " — the row was not loaded, the rest were")\n'),
