...
| step | what | state |
|---|---|---|
| 1 | policy layer + tests that can fail + PROVENANCE | **built 2026-09-01; REFUTED the same night by an outside seat (17 of 19 new bypass attempts through); REPAIRED 2026-09-02; REFUTED AGAIN by a second seat on the repair (12 of 27 new attempts through, 5 of 12 fix rows not holding as stated); REPAIRED AGAIN 2026-09-02** — both tables below. Gauges now (2026-09-03, from `GAUGES.md`): 227 passed, 99 skipped without a DSN (325 passed, 1 skipped, 5 min 0 s with one: the policy suite and the graph on SQLite and on PostgreSQL 18.6, plus the two-process restart test) · 44 checks · 44 killed by assertion · 0 killed only by a crash · 0 survived · 25 crashing test(s) alongside assertion kills · 37/37 scored attempts blocked · 1 out of scope, shown · 49/49 sabotages caught by at least one gauge. Trust boundary declared in `README.md`, restated to what the code supports. |
...
| ~~Client simulation 2~~ **SAT 2026-09-03, folded** — simulation 3 | BUILDER, then ✋ for what it raises | simulation 2 took the integrator's shape (its own section); what is left of the list below is the returning visitor after a real spin-down, 500 rows and an own key, XLSX out of an accounting program, and two people in one session. Simulation 1 (2026-09-03, its own section) took the shape the demo's page addresses: one bookkeeper, one folder of invoices, ten minutes. The shapes it did not: someone who comes back a week later to a slept instance; someone with 500 rows and their own Groq key; two people in one session; someone who uploads XLSX out of their accounting program; and the buyer the findings point at — whoever is about to give an assistant write access to their own records and is reading this to decide whether the boundary is real. One session, one shape, one record in STATUS.md each time |
...
- **May an auto-approved note assert a customer's claim as fact?** R4 asked whether it may; client
  ...
  says which notes are the assistant's, in the assistant's name, to whoever reads it. It is (c)
