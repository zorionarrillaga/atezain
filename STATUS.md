...
| 3 | red-team with promptfoo, boundary ON/OFF, numbers with intervals | not started — `PLAN.md` §3. **UNBLOCKED 2026-09-02: `GROQ_API_KEY` exists in the owner's shell profile and answers `/models` with 200; `openai/gpt-oss-120b` is served (14 models).** BUILDER builds against the stub, then runs for real; ⚖ the case texts |
