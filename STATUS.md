...
| R2-D1 | **the deployed half is NOT met.** The service is at `4b7bd10`, before the fold; `autoDeploy` is false, so a push is not a deploy. It watched a live `add_note` sit `approved` and never be written, with the chain verifying and no anomaly — round 1's D1, in public, a day after the fold was called folded. Round 1's second disjunct (*disclose the divergence*) was not taken either, so `README.md` and `STATUS.md` asserted, beside the live URL, something false of the thing at it | **disclosed, in both, at the point each names the URL** — what the running service is, that it still has the defect, and that it changes only when the owner deploys by hand. ✋ **the deploy itself is his, that day**; the code is ready and measured. Not made an anomaly, for the reason already recorded: approved-and-awaiting-execution is the ordinary resting state between `approve` and `record` in step 7's CLI, and the seat did not engage with that reason | `README.md` ¶ Status; the step-2 and S6-D1 rows above |
